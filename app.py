...
from Operations.email_operations import LANGCHAIN_TOOLS as EMAIL_TOOLS  # Import email tools
from system_prompt import SYSTEM_PROMPT

load_dotenv()

# Combine all tools
ALL_TOOLS = FILE_TOOLS + EMAIL_TOOLS

# Page config
st.set_page_config(
    page_title="File System & Email Agent",
    page_icon="📁",
    layout="wide"
)

st.title("📁 File System & Email Agent with LangGraph")
st.markdown("*AI-powered file and email operations with human approval*")
...
#   HELPER FUNCTIONS
# ======================
def extract_clean_content(content) -> str:
...
def save_session_to_mongodb(collection, thread_id: str, state):
    ...
    try:
        ...
        for msg in all_messages:
            msg_data = {
                "type": msg.type,
                "content": extract_clean_content(msg.content),
            ...
            if isinstance(msg, AIMessage):
                ...
                user_inputs.append(extract_clean_content(msg.content))
...
for msg in st.session_state.messages:
    if msg["role"] == "user":
        ...
        with st.chat_message("assistant"):
            # Clean display - only show text content
            clean_content = extract_clean_content(msg["content"])
            if clean_content:
                st.write(clean_content)
    elif msg["role"] == "tool":
        with st.chat_message("assistant", avatar="🔧"):
            # Clean tool result display
            clean_content = extract_clean_content(msg["content"])
            if clean_content:
                ...
                for event in st.session_state.graph.stream(
                    ...
                    if messages:
                        ...
                        if isinstance(last_msg, ToolMessage):
                            clean_content = extract_clean_content(last_msg.content)
                            if clean_content:
...
if not st.session_state.awaiting_approval:
    user_input = st.chat_input("Enter your file or email operation request...")
        ...
        try:
            ...
            for event in st.session_state.graph.stream(
                ...
                if messages:
                    ...
                    # Add assistant response with clean content
                    elif isinstance(last_msg, AIMessage) and last_msg.content:
                        clean_content = extract_clean_content(last_msg.content)
                        if clean_content:
                            st.session_state.messages.append({
